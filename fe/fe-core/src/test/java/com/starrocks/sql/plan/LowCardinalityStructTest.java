...
public class LowCardinalityStructTest extends PlanTestBase {
    ...
    public void testExpressionNonDictField() throws Exception {
        String sql = """
                ...
                """;
        String plan = getVerboseExplain(sql);
        String expected = "5 <-> row[([6: VARCHAR_COL, INT, true], [4: INTEGER_COL, INT, true]); args: INT,INT; " +
                "result: struct<`col1` int(11), `col2` int(11)>; args nullable: true; result nullable: true]" +
                ".col2[true]";
        Assertions.assertTrue(plan.contains(expected), plan);
    }
