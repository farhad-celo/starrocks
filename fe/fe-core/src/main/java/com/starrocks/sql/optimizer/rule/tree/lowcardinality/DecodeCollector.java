...
public class DecodeCollector extends OptExpressionVisitor<DecodeInfo, DecodeInfo> {
    ...
    public static final Set<String> LOW_CARD_AGGREGATE_FUNCTIONS_WITH_ENCODED_OUTPUT = Sets.newHashSet(
            FunctionSet.MAX, FunctionSet.MIN, FunctionSet.ANY_VALUE, FunctionSet.ARRAY_AGG);
    ...
    private boolean checkDependOnExpr(int cid, Collection<Integer> checkList) {
        ...
        if (define instanceof CallOperator && FunctionSet.ARRAY_AGG.equals(((CallOperator) define).getFnName())) {
            Map<String, ColumnRefOperator> fieldsMap = structRefToFieldUseStringRef.get(cid);
            Preconditions.checkState(fieldsMap != null && fieldsMap.containsKey("col1"));
            return checkDependOnExpr(fieldsMap.get("col1").getId(), checkList);
        }
    ...
    private boolean shouldProcessAggFunction(CallOperator agg) {
        ...
        if (isArrayAgg) {
            // ARRAY_AGG dictification is only supported for string inputs for aggregation.
            return agg.getChildren().stream().allMatch(c -> c.isColumnRef() || c.isConstantRef())
                    && agg.getFunction().getArgs()[0].isStringType();
        }
    ...
    public DecodeInfo visitPhysicalHashAggregate(OptExpression optExpression, DecodeInfo context) {
        ...
        for (ColumnRefOperator key : aggregate.getAggregations().keySet()) {
            ...
            if (!info.inputStringColumns.containsAll(value.getUsedColumns())
                    && !FunctionSet.ARRAY_AGG.equals(value.getFnName())) {
                continue;
            } else if (FunctionSet.ARRAY_AGG.equals(value.getFnName())) {
                // For ARRAY_AGG, we should process if the first input can be dictified
                Preconditions.checkState(value.getChild(0).isColumnRef());
                if (!info.inputStringColumns.contains(value.getChild(0).cast())) {
                    continue;
                }
            }
            // aggregate ref -> aggregate expr
            stringAggregateExpressions.computeIfAbsent(key.getId(), x -> Lists.newArrayList()).add(value);
            // min/max should replace to dict column, count/count distinct don't need
            if (LOW_CARD_AGGREGATE_FUNCTIONS_WITH_ENCODED_OUTPUT.contains(value.getFnName())) {
                info.outputStringColumns.union(key.getId());
                ...
                if (aggFn.getIntermediateTypeOrReturnType().isStructType()
                    ...
                    // Works for ARRAY_AGG and ANY_VALUE which are currently the only Aggregate functions generating
                    // structs. This will not work if the struct is not generated from the input fields.
                    ...
                    Map<String, ColumnRefOperator> fieldsData = Maps.newHashMap();
